package net.mcreator.creativeworld.mixin;
...
import net.mcreator.creativeworld.CreativeWorldModPlayerAnimationAPI;
...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (master == null)
			master = "creative_world";
		if (!master.equals("creative_world"))
			return;
		...
		if (entityIn instanceof Player player_)
			...
			return;
		CreativeWorldModPlayerAnimationAPI.PlayerAnimation animation = CreativeWorldModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (ageInTicks <= 0)
			return;
		if (!master.equals("creative_world")) {
			if (!CreativeWorldModPlayerAnimationAPI.animations.isEmpty())
				CreativeWorldModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			CreativeWorldModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		CreativeWorldModPlayerAnimationAPI.PlayerAnimation animation = CreativeWorldModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = CreativeWorldModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				CreativeWorldMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			CreativeWorldModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, CreativeWorldModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			CreativeWorldModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = CreativeWorldModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress);
			if (rotation != null) {
			...
			Vec3 position = CreativeWorldModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = CreativeWorldModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress);
			if (scale != null) {
